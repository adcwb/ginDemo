...
func HttpClient(urls, method, data string, Token string) (body []byte, err error) {
	...
	zap.L().Debug("HttpClient通用日志输出：",
		zap.String("urls", urls),
		zap.String("method", method),
		zap.String("data", data),
		zap.String("Token", Token),
		zap.String("startData", time.Now().Format("2006-01-02 15:04:05")))
	...
	// 将获取到的token返回出去
	zap.L().Debug("HttpClient通用日志输出结束：", zap.String("请求返回body", string(body)), zap.String("endData", time.Now().Format("2006-01-02 15:04:05")))
	return body, err
